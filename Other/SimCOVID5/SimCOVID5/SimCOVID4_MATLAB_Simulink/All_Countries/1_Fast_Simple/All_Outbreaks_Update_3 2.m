...
%==========================================================================
function Parameters = All_Outbreaks_Update_3
SearchCountry = 'germany';   % Enter the country name here (case insensitive). Use under-line dash "_" for a name with more than one part (ex: South_Africa, south_africa)
...
t0=findchangepts(Im,'Statistic','linear','MaxNumChanges',4);                 % Find 4 points on x-axis that have major change in y (used for the initial values)
if length(t0)==4                                                             % If the number of changes is 4
    t1=t0(1); t2=t0(2); t3=t0(3); t4=t0(4);
elseif length(t0)==3                                                         % If the number of changes is 3
    t1=0; t2=t0(1); t3=t0(2); t4=t0(3);
elseif length(t0)==2                                                         % If the number of changes is 2
    t1=0; t2=0; t3=t0(1); t4=t0(2);
elseif length(t0)==1                                                         % If the number of changes is 1
    t1=0; t2=0; t3=0; t4=t0(1);
else                                                                         % If no major changes in y values
    t1=0; t2=0; t3=0; t4=0;
end
N0 = SelectedRowsFlipped.popData2018; N = N0(1);                             % Population
              %--------beta 1-5--------  ---time 1-4---  a    gamma
Parameters0 = [0.2; 0.4; 0.5; 0.4; 0.2;  t1; t2; t3; t4; 0.1; 0.1];          % Initial values for 'lsqcurvefit' (Any change is significantly affecting the solution).
...
[Parameters] = SIR_Model;
    function [Parameters] = SIR_Model
        options = optimoptions('lsqcurvefit','Algorithm','trust-region-reflective','Display', 'iter','FunctionTolerance',1e-15,'MaxFunctionEvaluations',100000*2,'MaxIterations',2000,'OptimalityTolerance',1e-15); % Increase iterations if the results are not suffiecent (algorithms: levenberg-marquardt, 'trust-region-reflective')
              %-----------beta 1-5----------     ------------------------time 1-4----------------------  a   gamma
        lb = [ 0.01; 0.01;  0.01;  0.01; 0.01;   0;            0;            0;              0;          -1; 0.01];      % minimum values for the parameters (a must). The sigmoid parameter (a) can be negative.
        ub = [ 10;   10;    10;    10;   10;     length(Cm)/4; length(Cm)/2; length(Cm)*3/4; length(Cm); 10; 1   ];      % maximum values for the parameters (optional but preferred to be set)
        ...
        %================================Plotting I========================
        figure('Color',[1 1 1],'units','normalized','outerposition',[0 0 1 1])
        plot(Im,'p','LineWidth',1,'LineStyle',':','MarkerSize',8)
        hold on
        plot(Simulated_t,Simulated_I, 'linewidth',2)
        grid on; grid minor; set(gca,'fontsize',16);
        xticks(0:Simulation_length/10:Simulation_length);
...
%============Solving SIR with One Output (Needed for lsqcurvefit)==========
    function SIRout1 = SolveSIR1(Parameters,Simulated_t)
        ...
        function dSIR=DEs(Simulated_t,x)
            betaf1  = Parameters(1)*abs((sigmoid(Simulated_t,0,            Parameters(10)) - sigmoid(Simulated_t,Parameters(6),Parameters(10)))); % step function 1
            betaf2  = Parameters(2)*abs((sigmoid(Simulated_t,Parameters(6),Parameters(10)) - sigmoid(Simulated_t,Parameters(7),Parameters(10)))); % step function 2
            betaf3  = Parameters(3)*abs((sigmoid(Simulated_t,Parameters(7),Parameters(10)) - sigmoid(Simulated_t,Parameters(8),Parameters(10)))); % step function 3
            betaf4  = Parameters(4)*abs((sigmoid(Simulated_t,Parameters(8),Parameters(10)) - sigmoid(Simulated_t,Parameters(9),Parameters(10)))); % step function 4
            betaf5  = Parameters(5)*(sigmoid(Simulated_t,Parameters(9),Parameters(10)));
            betaf   = betaf1+betaf2+betaf3+betaf4+betaf5;
            dxdt    = zeros(4,1);
            dxdt(1) = -betaf/N*x(1)*x(2);                            % DE-1: S
            dxdt(2) = betaf/N*x(1)*x(2)-Parameters(11)*x(2);         % DE-2: I
            dxdt(3) = Parameters(11)*x(2);                           % DE-3: R
...
%=================Solving SIR with All Outputs (for Plotting)==============
    function [tout, SIRout1] = SolveSIR2(Parameters,Simulated_t)
        ...
        function dSIR=DEs(Simulated_t,x)
            betaf1  = Parameters(1)*abs((sigmoid(Simulated_t,0,            Parameters(10)) - sigmoid(Simulated_t,Parameters(6),Parameters(10))));
            betaf2  = Parameters(2)*abs((sigmoid(Simulated_t,Parameters(6),Parameters(10)) - sigmoid(Simulated_t,Parameters(7),Parameters(10))));
            betaf3  = Parameters(3)*abs((sigmoid(Simulated_t,Parameters(7),Parameters(10)) - sigmoid(Simulated_t,Parameters(8),Parameters(10))));
            betaf4  = Parameters(4)*abs((sigmoid(Simulated_t,Parameters(8),Parameters(10)) - sigmoid(Simulated_t,Parameters(9),Parameters(10))));
            betaf5  = Parameters(5)*(sigmoid(Simulated_t,Parameters(9),Parameters(10)));
            betaf   = betaf1+betaf2+betaf3+betaf4+betaf5;
            dxdt    = zeros(4,1);
            dxdt(1) = -betaf/N*x(1)*x(2);                            % DE-1: S
