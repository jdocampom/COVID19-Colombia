...
bar(Im); legend('reported cases');
hold on
plot(t,I, 'linewidth',3); hold on
Legend=cell(n+1,1);
for m=1:n+1
    if m==1
        Legend{m}='reported cases';
    else
        Legend{m}=strcat('scenario', num2str(m-1));
    end
end
legend(Legend);
    ...
    function [tout, SIRout1] = SolveSIR2(Parameters,Simulated_t)
        ...
        function dSIR=DEs(Simulated_t,x)
            ...
            if Simulated_t < length(Cm)
                ...
                betag=ones(1,n).*[0.95 0.975 1 1.025 1.05];                                         % This is the change percentage of the beta function
            end
            dxdt    = zeros(4,1);
