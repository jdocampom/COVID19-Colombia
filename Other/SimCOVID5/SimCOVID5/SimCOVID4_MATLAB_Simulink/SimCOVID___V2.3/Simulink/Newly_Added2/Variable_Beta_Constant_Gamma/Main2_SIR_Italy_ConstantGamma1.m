...
% Case: Italy Outbreak - Constant Gamma, Variable beta
...
gamma   = 0.204682532425745; 
beta1   = 0.392927835333545;
beta2   = 0.310143549986011;
beta3   = 0.175218449871955;
...
isModelOpen = bdIsLoaded('SIR_Italy_ConstantGamma10'); % We need this later on to close the parallel loop
open_system('SIR_Italy_ConstantGamma10');
beta_new=1;
beta_step = beta_new*[0.975 1 1.025];   % These are normalized beta changes (10%)
numSims = length(beta_step);
for i = numSims:-1:1
    in(i) = Simulink.SimulationInput('SIR_Italy_ConstantGamma10');
    in(i) = in(i).setBlockParameter('SIR_Italy_ConstantGamma10/Value2', 'Value', num2str(beta_step(i)));
end
in = in.setBlockParameter('SIR_Italy_ConstantGamma10/Sigmoidal MF1','a',num2str(-a));
in = in.setBlockParameter('SIR_Italy_ConstantGamma10/Sigmoidal MF1','c',num2str(st1));
in = in.setBlockParameter('SIR_Italy_ConstantGamma10/Sigmoidal MF2','a',num2str(a));
in = in.setBlockParameter('SIR_Italy_ConstantGamma10/Sigmoidal MF2','c',num2str(st1));
in = in.setBlockParameter('SIR_Italy_ConstantGamma10/Sigmoidal MF3','a',num2str(a));
in = in.setBlockParameter('SIR_Italy_ConstantGamma10/Sigmoidal MF3','c',num2str(st2));
in = in.setBlockParameter('SIR_Italy_ConstantGamma10/gamma','Value',num2str(gamma));
in = in.setBlockParameter('SIR_Italy_ConstantGamma10/beta1','Gain',num2str(beta1));
in = in.setBlockParameter('SIR_Italy_ConstantGamma10/beta2','Gain',num2str(beta2));
in = in.setBlockParameter('SIR_Italy_ConstantGamma10/beta3','Gain',num2str(beta3));
in = in.setBlockParameter('SIR_Italy_ConstantGamma10/g','Gain',num2str(g));
in = in.setBlockParameter('SIR_Italy_ConstantGamma10/Value1','Value',num2str(1));
in = in.setBlockParameter('SIR_Italy_ConstantGamma10/Switch','Threshold',num2str(st3));
in = in.setBlockParameter('SIR_Italy_ConstantGamma10/Population','Value',num2str(P));
in = in.setBlockParameter('SIR_Italy_ConstantGamma10/I0','Value',num2str(I0));
in = in.setBlockParameter('SIR_Italy_ConstantGamma10/R0','Value',num2str(R0));
...
if(~isModelOpen)
    close_system('SIR_Italy_ConstantGamma10', 0);
end
