...
function [Parameters] = SIR_Parameters_Italy2
...
st1 = 40; st2 = 52;                 % Step times for the sigmoid function (chosen empirically by observing Cm plot)
x0  = [S0; I0; R0; C0 ];            % Initial values for the states (S,I,R) used in 'ode45'
...
[Parameters] = SIR_Model;
    function [Parameters] = SIR_Model
        ...
        Im  = flip(I1);  Im=Im(32:end);            %  Neglecting zero elements and collecting total sum of infectious before simulation start time (I0=sum(Im(1:53)))
        Cm  = cumsum(Im);                          %  Cumulative I
        ...
        legend({'simulated cumulative infectious','reported cases'}, 'FontSize',12, 'location', 'northwest');
        dateaxis('x', 6, '31-Jan-2020')
        %============================Plotting I============================
        figure('Color',[1 1 1],'units','normalized','outerposition',[0 0 1 1])
