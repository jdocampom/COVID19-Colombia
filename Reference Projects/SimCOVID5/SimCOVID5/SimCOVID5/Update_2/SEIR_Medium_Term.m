...
%---------------------------End of Normalize-------------------------------
Simulation_length = 1.5*length(Im);                                         % Simulation time (day). Note: use this carefully as it is now part in the beta function.
Parameters000=Im4'; Parameters01= Parameters000(t0);
Parameters0 = [Parameters01; (t0(1:end-1))'; min(Parameters01); Simulation_length*1; 0; min(Parameters01); min(Parameters01)]';       % Total initial values for 'lsqcurvefit'.
...
lb = [ 0.01*ones(1,length(t0))   t_lb     0.25   Simulation_length*1/4   0    1/14 0.01 ];               % minimum values for the parameters (a must). The sigmoid parameter (a) can be negative.
ub = [ 10.0*ones(1,length(t0))   t_ub     0.75   Simulation_length*4/4   0.1  1/2  10.0 ];               % maximum values for the parameters (optional but preferred to be set)
