...
% Note 2: You can control your curve-fitting by editing (1) 'MaxNumChanges' and 'Statistic' @line 36 and (2) the initial values (Parameters0 vector) and their limits.
...
% Note 5: You only need to enter the country name @line-19 and run the model.
% Keynote 1: The number of sigmoid steps @line-36 plays a significant role especially in tracking data (high value gives smoother results and visa versa). For estimation, start with a low value of "MaxNumChanges" > 1, that is, 2, 3, 4, 5, and so on. You can increase it if you have enough data or the outbreak passed the peak. Typical values 3-5.
% Keynote 2: You can switch between the methods of "findchangepts" @line-36 ("rms", "linear", "std", or "mean"). You can also change the simulation length @line-45 to  get more time line view of the prediction.
% Keynote 3: This is a generalized method for all outbreaks (current and future). For this reason, a "smooth" function @ line 39 is used to prevent some unexpected data. You can remove it for your case to see if that affects.
% Keynote 4: Change the number of iterations (and other setting options) of the solver @line-69 for better results if needed.
...
%==========================================================================
function Parameters = SEIR_Generalized_Variable_Control
clc;
c=0.1; % Control amount of infection. Range varies between [0 1]. This parameter is used to control the infection and estimate the outbreak with this amount of control. With c=0, there is no control over the infection (0%) and the program uses the last infection rate to predict the future outbreak. With c>0, the infection quantity reduces as a function of this value.
...
t0=findchangepts(Im,'Statistic','linear','MaxNumChanges',3);                 % Find all points on x-axis (time) that have major change in y (it is also number of sigmoid functions). Set the value to any number such as 3, 5, 10,... or "length(Cm)". You can also enter t0 manually as a vector.
%--------------------End of number of sigmoid steps------------------------
N0 = SelectedRowsFlipped.popData2018; N = N0(1);                             % Population
Im1=(smooth(Im))'; Im1(Im1==0)=1;
Im2=[Im1(2:end) Im1(end)];  Im3=Im2./Im1;                                    % Find all initial values for the beta gains that are close to confirmed cases ratio (data) with a normalized range (use either "normalize" function or by dividing by max value
%-----------------------Normalize Im (two methods)-------------------------
% Im4=normalize(Im3,'range',[0.1 1]);                                        % Method 1 (change the range to get better results, if possible)
...
%--------------beta gains--------time-----------a---expoent-----dc----lambda-------------gamma(gain, step time,    a,   dc)
Parameters0 = [1./Parameters01; (t0(1:end-1))'; 1;  length(Cm); 0.0;  1/mean(2,14);      1;          length(Cm)/2; 0.1; 0.0]';       % Total initial values for 'lsqcurvefit'.
...
lb = [ 0.1*ones(1,length(t0))  t_lb     0.1     0.0*length(Cm)   0.0     1/14      0.1  0           0.1  0.00];    % minimum values for the parameters (a must). The sigmoid parameter (a) can be negative.
ub = [ 1.0*ones(1,length(t0))  t_ub     1.0     1.0*length(Cm)   0.1     1/2       1    length(Cm)  1.0  0.01];    % maximum values for the parameters (optional but preferred to be set)
...
[Parameters] = SEIR_Model;
    function [Parameters] = SEIR_Model
        ...
        [Parameters] = lsqcurvefit(@SolveSEIR1,Parameters0',tm',Cm', lb, ub, options);   % Least-Square curve fitting
...
for k = 1:3
    [tout,Simulated_SEIR(:,k)] = SolveSEIR2(Parameters, Simulated_t);       % Solve SEIR with three values of beta (standard, plus minus a percentage of your choice)
end
%=============================Plotting C===================================
t=tout;
...
%============Solving SEIR with One Output (Needed for lsqcurvefit)=========
    function SEIRout1 = SolveSEIR1(Parameters,Simulated_t)
        ...
        function dSEIR=DEs(Simulated_t,x)
            %--"sigmoid" is a smooth step-function used for the beta (infection) anf gamma (recovery) functions
            ...
            betaf    = (betaf1+betaf2+betaf3)*exp(-c*Simulated_t/(Parameters(2*length(t0)+1))) + Parameters(2*length(t0)+2);                                                            % Infection rate
            lambdaf  = Parameters(2*length(t0)+3);                                                                                                                                    % Exposed rate
            gammaf   =     Parameters(2*length(t0)+4).*   ((sigmoid(Simulated_t,Parameters(2*length(t0)+5)             ,Parameters(2*length(t0)+6) ))) + Parameters(2*length(t0)+7);  % Recovery rate
            %---------------------------DEs--------------------------------
            dxdt    = zeros(4,1);
        ...
        SEIRout1 = cumsum(SEIRout0(:,3)) ;                                  % Pick a state variable (here, C (or cumsum(I)) is selected to compare it with Cm and minimize the difference)
    end
