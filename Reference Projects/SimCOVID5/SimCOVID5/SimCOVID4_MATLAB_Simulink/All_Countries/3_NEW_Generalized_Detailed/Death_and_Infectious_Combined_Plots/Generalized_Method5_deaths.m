...
t0=findchangepts(Dm,'Statistic','linear','MaxNumChanges',5);                % Find all points on x-axis (time) that have major change in y (it is also number of sigmoid functions). Set the value to any number such as 3, 5, 10,... or "length(Cm)". You can also enter t0 manually as a vector.
%--------------------End of number of sigmoid steps------------------------
N0 = SelectedRowsFlipped.popData2018; N = N0(1);                             % Population
Dm1=(smooth(Dm))'; Dm1(Dm1==0)=1;
Dm2=[0 Dm1(1:end-1)];  Dm3=Dm2./Dm1;                                         % Find all initial values for the beta gains that are close to confirmed cases ratio (data) with a normalized range (use either "normalize" function or by dividing by max value
%-----------------------Normalize Dm (two methods)-------------------------
% Dm4=normalize(Dm3,'range',[0.1 0.5]);                                      % Method 1 (change the range to get better results, if possible)
Dm4=Dm3./max(Dm3);                                                           % Method 2
%---------------------------End of Normalize-------------------------------
Parameters000=Dm4'; Parameters01= Parameters000(t0);
Parameters0 = [Parameters01; (t0(1:end-1))'; min(Parameters01); min(Parameters01)]';                    % Total initial values for 'lsqcurvefit'.
I0=Dm(1); x0  = [N-I0; I0; 0; I0];                                           % Initial values for the states (S,I,R, C) used in 'ode45'
DateStart0 = SelectedRowsFlipped.dateRep; DateStart= DateStart0(1);          % First date with confirmed case(s)
%----------------------------Important Note--------------------------------
%--------------------------Simulation Length-------------------------------
%-----------Keynote1: use "length(Dm)" for tracking existing data----------
%--------Keynore2: use > "length(Dm)" for estimating future outbreaks------
%--------------Keynote3: for estimating future, use less t0----------------
Simulation_length = 1.5*length(Dm);                                            % Simulation time (day)
...
[Parameters] = SIR_Model;
    function [Parameters] = SIR_Model
        ...
        %-------------------------Important Note---------------------------
        %--Compare either (1) Dm (or gradient(Cm)) with simulated I (SIRout0(:,2)) or (2) Cm with simulated C (SIRout0(:,4))
...
%============Solving SIR with One Output (Needed for lsqcurvefit)==========
    function SIRout1 = SolveSIR1(Parameters,Simulated_t)
        ...
        SIRout1 = (SIRout0(:,2)) ;                                   % Pick a state variable (here, I is selected to compare it with Dm and minimize the difference)
    end
