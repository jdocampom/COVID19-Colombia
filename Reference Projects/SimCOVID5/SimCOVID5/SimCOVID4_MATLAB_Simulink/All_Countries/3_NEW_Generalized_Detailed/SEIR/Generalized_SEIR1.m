...
%==========================================================================
function Parameters = Generalized_SEIR1
clc;
...
Parameters0 = [Parameters01; (t0(1:end-1))'; min(Parameters01); 1/6; min(Parameters01)]';       % Total initial values for 'lsqcurvefit'.
I0 = Im(1); S0 = N-I0;  E0 = 0; R0 = 0; x0  = [S0; E0; I0; R0];              % Initial values for the states (S,E,I,R) used in 'ode45'
...
Simulation_length = 1*length(Im);                                             % Simulation time (day)
%-------------------------End of Simulation Time---------------------------
step_size = 1;                                                                % To give a smooth curve, choose a step size less than 1 (0.1 is preferred)
...
lb = [ 0.01*ones(1,length(t0))   t_lb     0.25  1/14   0.01 ];               % minimum values for the parameters (a must). The sigmoid parameter (a) can be negative.
ub = [ 10.0*ones(1,length(t0))   t_ub     0.75  1/2    1.00 ];               % maximum values for the parameters (optional but preferred to be set)
...
[Parameters] = SEIR_Model;
    function [Parameters] = SEIR_Model
        ...
        %==========================Plotting================================
        [~,Simulated_SEIR] = SolveSEIR2(Parameters, Simulated_t);
        Simulated_S  = Simulated_SEIR(:,1);
        Simulated_E  = Simulated_SEIR(:,2);
        Simulated_I  = Simulated_SEIR(:,3);
        Simulated_R  = (Simulated_SEIR(:,4));
        Simulated_C  = cumsum(Simulated_SEIR(:,3));
        Simulated_N  = Simulated_S+Simulated_E+Simulated_I+Simulated_R;
        %===========================Plotting N=============================
        ...
        ylabel('N = S + E + I + R (person)','FontSize',16,'FontWeight','bold');
        ...
        %===========================Plotting E=============================
        figure('Color',[1 1 1],'units','normalized','outerposition',[0 0 1 1])
        plot(Simulated_t,Simulated_E, 'linewidth',2)
        grid on; grid minor; set(gca,'fontsize',16);
        xticks(0:Simulation_length/10:Simulation_length);
        xlabel('Time (day)','FontSize',16,'FontWeight','bold');
        xlim([0 Simulation_length])
        ylabel('Exposed (person)','FontSize',16,'FontWeight','bold');
        dateaxis('x', 6, DateStart)
        %===========================Plotting R=============================
        figure('Color',[1 1 1],'units','normalized','outerposition',[0 0 1 1])
        ...
        %==========================Plotting I (bar)=======================
        figure('Color',[1 1 1],'units','normalized','outerposition',[0 0 1 1])
        bar(tm, Im); hold on; bar(tm, Dm);  hold on;
        ...
        legend({'Daily infectious (reported cases)','Daily deads (reported cases)','Daily infectious (simulated)'}, 'FontSize',12, 'location','northwest');
        dateaxis('x', 6, DateStart)
        %==========================Plotting C (bar)========================
        figure('Color',[1 1 1],'units','normalized','outerposition',[0 0 1 1])
        bar(tm,cumsum(Im)); hold on; bar(tm,cumsum(Dm));  hold on;
        ...
        legend({'Cumulative infectious (reported cases)','Cumulative deads (reported cases)','Cumulative infectious (simulated)'}, 'FontSize',12, 'location','northwest');
        dateaxis('x', 6, DateStart)
        %================================Plotting I========================
        figure('Color',[1 1 1],'units','normalized','outerposition',[0 0 1 1])
...
%============Solving SEIR with One Output (Needed for lsqcurvefit)==========
    function SEIRout1 = SolveSEIR1(Parameters,Simulated_t)
        ...
        function dSEIR=DEs(Simulated_t,x)
            ...
            lambdaf=Parameters(2*length(t0)+1);                             % Exposed rate
            gammaf=Parameters(2*length(t0)+2);                              % Recovery rate
            %---------------------------DEs--------------------------------
            dxdt    = zeros(4,1);
...
%=================Solving SEIR with All Outputs (for Plotting)==============
...
betaf3   =     Parameters(length(t0)    ).*   ((sigmoid(Simulated_t,Parameters(2*length(t0)-1)             ,Parameters(2*length(t0)) ))) ;
betaf    = betaf1+betaf2+betaf3;
gammaf=Parameters(2*length(t0)+2);
R0=betaf./gammaf;
