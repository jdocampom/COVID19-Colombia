...
%==========================================================================
function Parameters = Generalized_SEIR4
clc;
...
Simulation_length = 1.25*length(Im);                                         % Simulation time (day)
...
ub = [ 10.0*ones(1,length(t0))   t_ub     0.75  1/2    1.00 ]; 
...
%==========================Solving SEIR "n" Times===========================
n=5;                                                                          % number of scenarios (check line-152 as well whenever you edit "n")
Simulated_SEIR=zeros(length(Simulated_t),n);
for k = 1:n
...
%==========================Plotting all I's================================
t=tout;
I=zeros(length(tout),n);
for r=1:n
    I(:,r) = Simulated_SEIR(:,r);
end
figure('Color',[1 1 1],'units','normalized','outerposition',[0 0 1 1])
bar(tm, Im); hold on; bar(tm, Dm);                                           %----if you want to revesrse death and infectious stacked bars, use this: bar(tm, [Im; Dm],'stacked'). If you want to plot only Im, use this: bar(Im)
hold on
plot(t,I, 'linewidth',3); hold on
Legend=cell(n+2,1);
for m=1:n+2
    if m==1
        Legend{m}='reported infection cases';
    elseif m==2
        Legend{m}='reported death cases';
    else
        Legend{m}=strcat('scenario', num2str(m-2));
    end
end
legend(Legend);
...
%===================Solving SEIR Three Times for Plotting===================
    function [tout, SEIRout1] = SolveSEIR2(Parameters,Simulated_t)
        ...
        function dSEIR=DEs(Simulated_t,x)
            ...
            gammaf   = Parameters(2*length(t0)+2);                          % Recovery rate
            %---------------------------DEs--------------------------------
            if Simulated_t < length(Cm)
                betag=[ 1 1 1 1 1];
            else
                betag=ones(1,n).*[0.9 0.95 1.05 1.10 1 ];                   % This is the change percentage of the beta function
            end
            dxdt    = zeros(4,1);
            S = x(1); E = x(2); I = x(3); % R = x(4); 
