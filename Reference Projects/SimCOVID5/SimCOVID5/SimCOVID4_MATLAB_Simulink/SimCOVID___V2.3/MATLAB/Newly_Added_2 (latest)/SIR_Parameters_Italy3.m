...
function SIR_Parameters_Italy3
...
betag = [0.985 1.0 1.015];                  % Beta change at time zero (remember the gamma here is an estimated constant value and the beta function is more senesitive to a step change).
...
%======================Curve Fitting Function (Called)=====================
    function [Parameters] = SIR_Model
        Parameters0 = [0.6; 0.5; 0.4; 0.8; 0.4];    % Initial values for 'lsqcurvefit'
...
%=================Solving SIR with All Outputs (for Plotting)==============
    function [tout, SIRout1] = SolveSIR2(Parameters,t)
        ...
        function dSIR=DEs(t,x)
            ...
            dxdt(2) = betag(i)*betaf/N*x(1)*x(2)-Parameters(5)*x(2); % DE-2: I  (beta has now a new gain "betag")
            dxdt(3) = Parameters(5)*x(2);                            % DE-3: R
